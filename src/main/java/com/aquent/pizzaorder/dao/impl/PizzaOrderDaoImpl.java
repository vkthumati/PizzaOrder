...
public class PizzaOrderDaoImpl implements IPizzaOrderDao {
	...
	public void loadPizzaOrdersData() {
		...
		if(StringUtils.isEmpty(soursePath) || StringUtils.isEmpty(destinationPath)) {
			throw new ApplicationException("PATH_ERROR", "Source Path or Destination Path Not Valid");
		}
		...
		try (	
			...
			throw new ApplicationException("FILE_OPERATIONS", e.getMessage());
		} 
	}
